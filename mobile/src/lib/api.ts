...
export interface SendDeviceTrustOtpPayload {
  ...
  isRooted?: boolean;
  isEmulator?: boolean;
  /** 11-char Android app hash — enables silent SMS Retriever auto-read */
  appHash?: string;
  /** 5-digit OTP generated by the app — backend sends this exact value via SMS */
  clientOtp?: string;
}
...
export interface VerifyDeviceTrustOtpPayload {
  ...
  isRooted?: boolean;
  isEmulator?: boolean;
  // Anti-replay: generated by app on each request
  nonce: string;
  timestamp: number;
}

export interface InitSmsVerifyPayload {
  deviceId: string;
  fingerprintHash: string;
  simPhoneNumber?: string;
  simCarrier?: string;
...
export interface SmsVerifySession {
  sessionToken: string;
  targetNumber: string;
  smsBody: string;
  expiresAt: number;
...
export interface SmsVerifyStatusResponse {
  verified: boolean;
  expired: boolean;
}
...
export const deviceTrustApi = {
   ...
   * Returns { sessionToken, targetNumber, smsBody, expiresAt }
   * App must send an SMS to targetNumber with body smsBody.
   */
  initSmsVerify: (payload: InitSmsVerifyPayload) =>
    api.post('/auth/device-trust/init-sms-verify', payload),

  /**
   * Poll SMS verification status after app has sent the verification SMS.
   * Returns { verified, expired }
   */
  smsVerifyStatus: (sessionToken: string) =>
    api.get('/auth/device-trust/sms-verify-status', { params: { sessionToken } }),

  /**
   * Send OTP to enrolled phone to start device verification (OTP fallback).
   ...
   * Includes nonce + timestamp for anti-replay protection.
